 ...
 */
NetSharedPtr
Net::findSubnet(const std::string &inName) throw(Error) {
    NetSharedPtr subnet;
 ...
 */
void
Net::removeSubnet(const std::string &inName) throw(Error) {
    if( inName.empty() )
    {
        Error e( eMessageIdErrorEmptyItemName,
            __FUNCTION__, __FILE__, __LINE__ );
...
Net::setSubnets(
    ...
    std::vector< NetSharedPtr >::const_iterator subnet
                                        = inSource.begin();
    std::vector< NetSharedPtr >::const_iterator subEnd
                                        = inSource.end();
 ...
 */
PortSharedPtr
Net::findConnectedPort( const std::string &inName) throw(Error) {
    PortSharedPtr port;
    std::list< PortSharedPtr >::iterator it
        = find_if( mConnectedPorts.begin(), mConnectedPorts.end(),
            boost::bind<bool>( std::equal_to<std::string>(),
                boost::bind( boost::mem_fn( &Port::getName ), _1),
                                                    inName)); 
...
void
Net::getConnectedPorts(
    std::vector< PortSharedPtr > &outPorts,
    bool inSkipChildConnections ) const throw() {
    outPorts.insert( outPorts.end(),
            mConnectedPorts.begin(), mConnectedPorts.end() );
    return;
}


void
Net::removeConnectedPort( const PortSharedPtr &inPort) throw(Error) {
    std::list< PortSharedPtr >::iterator it
        = find( mConnectedPorts.begin(), mConnectedPorts.end(),
                inPort);
    if( it != mConnectedPorts.end() )
    {
...
void
Net::setConnectedPorts(
    const std::vector< PortSharedPtr > &inSource) throw(Error) {
    std::vector< PortSharedPtr >::const_iterator port
                                = inSource.begin();
    std::vector< PortSharedPtr >::const_iterator portEnd
                                = inSource.end();
...
void
Net::getConnectedPortLists(
    std::vector< PortListSharedPtr > &outPortList) const throw() {
    outPortList.insert( outPortList.end(),
            mConnectedPortLists.begin(), mConnectedPortLists.end() );
    return;
}

void
Net::addConnectedPortList(
...
void
Net::removeConnectedPortList(
        const PortListSharedPtr &inPortList) throw(Error) {
    std::list< PortListSharedPtr >::iterator it
        = find( mConnectedPortLists.begin(), mConnectedPortLists.end(),
                inPortList);
...
Net::findConnectedPortReference(
                const std::string &inName) throw(Error) {
    PortReferenceSharedPtr portRef;
    std::list< PortReferenceSharedPtr >::iterator it
        = find_if(
            mConnectedPortRefs.begin(), mConnectedPortRefs.end(),
            boost::bind<bool>( std::equal_to<std::string>(),
                boost::bind(
                    boost::mem_fn(&PortReference::getName ), _1),
                    inName)); 
...
void
Net::getConnectedPortRefs(
    std::vector< PortReferenceSharedPtr > &outPortRefs,
    bool inSkipChildConnections ) const throw() {
    outPortRefs.insert( outPortRefs.end(),
            mConnectedPortRefs.begin(), mConnectedPortRefs.end() );
    return;
}

void
Net::removeConnectedPortReference(
                const PortReferenceSharedPtr &inPortRef) throw(Error) {
    std::list< PortReferenceSharedPtr >::iterator it
        = find( mConnectedPortRefs.begin(), mConnectedPortRefs.end(),
                inPortRef );
    if( it != mConnectedPortRefs.end() )
    {
...
Net::serialize( Archive &ar, unsigned int ) {
    ...
    ar & boost::serialization::base_object< PropertyContainer >(
                                                           *this );
...
template void
Net::serialize<boost::archive::binary_iarchive>(
        boost::archive::binary_iarchive & ar, const unsigned int);

template void
Net::serialize<boost::archive::binary_oarchive>(
